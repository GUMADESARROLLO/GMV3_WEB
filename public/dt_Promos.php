...
$sql_query = "SELECT * FROM tbl_news";
...
if($total_records >= 1) {
    while ($link = mysqli_fetch_array($resouter, MYSQLI_ASSOC)){
        ...
        $StrPermisos ='
                                        ...
                                        <a href="edit-promo.php?banner_id='.$link['banner_id'].'">
                                            <i class="material-icons">mode_edit</i>
                                        </a>
                                                            
                                        <a href="delete-promo.php?banner_id='.$link['banner_id'].'" onclick="return confirm(\'¿Estás seguro de que deseas eliminar este banner?\')" >
        ...
        $set['data'][$i]['sku_name']        = $link['banner_sku']." - ".$link['sku_name'];
        $set['data'][$i]['banner_name']     = $link['banner_description'];
        $set['data'][$i]['banner_image']    = '<img src="upload/news/'. $link['banner_image'].'" width="200px" height="200px"/>';
